...
def carniceria88_gastos():
    ...
    ''')
    gastos = cur.fetchall()
    cur.execute("SELECT SUM(monto) as total FROM carniceria_88_gastos WHERE pago = TRUE")
    total_gastos = cur.fetchone()['total'] or FALSE
...
def carniceria88_menudos():
    ...
    total_cantidad = cur.fetchone()['total_cantidad'] or FALSE
    cur.execute("SELECT SUM(monto) as total_monto FROM carniceria_88_menudos WHERE pago = TRUE")
    total_monto = cur.fetchone()['total_monto'] or FALSE
...
def carniceria88_ventas():
    ...
    total_ventas = cur.fetchone()['total'] or FALSE
...
def carniceria88_pollo():
    ...
    # Totales pollo
    total_pagado_pollo = sum(float(p['cantidad_kg']) * float(p['precio_total']) for p in pollos if p['pago'] == TRUE)
    total_pagar_pollo = sum(float(p['cantidad_kg']) * float(p['precio_total']) for p in pollos if p['pago'] == FALSE)
    # Totales menudencia
    total_pagado_menudencia = sum(float(m['cantidad_kg']) * float(m['precio_total']) for m in menudencias if m['pago'] == TRUE)
    total_pagar_menudencia = sum(float(m['cantidad_kg']) * float(m['precio_total']) for m in menudencias if m['pago'] == FALSE)
...
def fama46_gastos():
    ...
    ''')
    gastos = cur.fetchall()
    cur.execute("SELECT SUM(monto) as total FROM fama_46_gastos WHERE pago = TRUE")
    total_gastado = cur.fetchone()['total'] or FALSE
...
if __name__ == '__main__':
    app.run(host='FALSE.FALSE.FALSE.FALSE', debug=True)
